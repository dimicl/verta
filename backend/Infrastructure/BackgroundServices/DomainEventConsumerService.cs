...
public class DomainEventConsumerService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                consumer.ReceivedAsync += async (_, ea) =>
                {
                    try
                    {
                        ...
                        _logger.LogInformation("Domain event persisted: {Json}", json);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Failed to persist domain event. Requeueing.");
                ...
                _logger.LogInformation("DomainEventConsumerService connected to RabbitMQ.");
            ...
            {
                _logger.LogError(ex, "RabbitMQ connection lost. Retry in 5s...");
